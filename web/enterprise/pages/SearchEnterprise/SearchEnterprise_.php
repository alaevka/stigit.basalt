...
['name_area','id_area','../../Include/showArea.php',20]
);
  ...
  switch(m_spisok[id.alt][0])
    {
	   ...
	   case 'name_area': { m_spisok[id.alt][4],"size=" + m_spisok[id.alt][3] + "&text=" + id.value + "&num=" + id.alt + doptext} break;
	 ...
	 <form  action="" method="get" name="SearchForm" target="_parent" id="SearchForm" style="padding:0px; margin:0px;">
		...
		<td >Страна</td>
		 ...
		 <tr class="rowList item">
		   ...
		   <td><input name="name_area" type="text" class="textFieldSelect" id="name_area" value="<? if ( !empty($name_area) ) echo htmlspecialchars($name_area) ?>" size="80" maxlength="200">
		   <input type="hidden" name="id_area" id="id_area" value="<? if ( !empty($id_area) ) echo htmlspecialchars($id_area) ?>" ></td>
		   </tr>	  
		 <tr class="rowList item">
		 	<td align="left">Недействующие</td>
	 		<td align="left"><input name="action" type="checkbox" id="action" value="1" <? if(!empty($action)) echo 'checked';?>></td>
		 </tr>
		 </tbody>
		 <tr class="rowList item">
	  ...
	  <td><div id="pfc_online" style="border: #072a66 1px solid;">
	    <table class="borderList" width="100%" border="0" cellspacing="1" cellpadding="4" style="border-width:0px;">
          ...
          <tr class='rowList item' style="cursor:pointer" onMouseOver="this.style.background = '#f0f5fa';" onMouseOut="this.style.background = '#e4eaf2';"
		   <? if(!$EnterpriseArr['ACTION'][$i]) echo'style="color:#999999"'; ?>
		  >
